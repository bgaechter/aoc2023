...
std::pair<CardHand, int> get_hand_and_bid(const std::string &line) {
	std::cout << line << '\n';
...
bool compare_hands(const CardHand &hand1, const CardHand &hand2) {
...
void solve_day07b(std::ifstream &infile) {

	infile.clear();  // clear fail and eof bits
	infile.seekg(0, std::ios::beg);
	std::string line;

	std::vector<std::pair<CardHand, int>> hands_and_bids;
